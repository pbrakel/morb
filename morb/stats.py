...
def pt_stats(rbm, v0_vmap, visible_units, hidden_units, persistent_vmap, beta, k=1):
    ...
    # TODO: ask sander if this makes sense with persistent cd like algorithms.
    h0_stats_vmap, h0_gibbs_vmap = gibbs_step(rbm, v0_vmap, hidden_units)
    ...
    def gibbs_hvh(*args):
        ...
        v1_activation_vmap = dict((v, v.activation(v1_in_vmap)) for v in visible_units)
        v1_stats_vmap, v1_gibbs_vmap = gibbs_step(rbm, v1_in_vmap, visible_units)

        h1_in_vmap = v1_gibbs_vmap.copy()

        h1_activation_vmap = dict((h, h.activation(h1_in_vmap)) for h in hidden_units)
        h1_stats_vmap, h1_gibbs_vmap = gibbs_step(rbm, h1_in_vmap, hidden_units)
